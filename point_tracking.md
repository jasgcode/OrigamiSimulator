...
For scan-state filtering, the default minimum threshold is **0.25**. Faces passing the backface and occlusion tests but with quality below this threshold are excluded from the visible set.

When progression building is enabled (`"buildProgressions"`), phase-2 live trajectory evaluation uses a stricter default quality floor of **0.6** if `minFaceQuality` is not provided. If `minFaceQuality` is explicitly set in config, that value is used for both scan-state filtering and phase-2 evaluation.
...
Run via `?benchmark=scan-bird`. Output is saved to `screenshots/<name>/scan.json`.

Note: if `"saveScanResult": false`, results are returned in-memory to the callback but no file is written.
...
## Greedy Suggested-Sequence Algorithm
...
This section describes how `suggestedSequences` are built from phase-1 scan data.

### Fixed anchor frame

For `suggestedSequences`, frame 0 is always `{ fold: 0, pov: "y" }` — the flat paper viewed from above. This gives the viewer an unambiguous reference before folding begins.
...
## Phase 2: Diverse Progressions (Live Evaluation)

When `"buildProgressions"` is enabled, scan mode also runs a second pass that evaluates candidate trajectories live in the simulator.

Key differences vs `suggestedSequences`:

- Candidate trajectories are generated from continuous POV endpoints and start from `iso` at the first fold step.
- Every step is validated against face quality and tracked-point constraints.
- Tracked-point evaluation supports:
  - `trackingEvalMode: "strictAllSteps"` (default) — required points must stay visible at every required step.
  - `trackingEvalMode: "finalStepOnly"` — intermediate misses tolerated; final step remains strict.
- Point-separation checks always apply at final step, and can be applied at all steps with `enforceSeparationAllSteps: true` (in strict mode).
- Runtime controls include `maxTrajectoryCandidates`, `phase2LogEveryMs`, and `phase2StallWarnMs`.

The output includes `diverseProgressions` and `progressionStats` in addition to `suggestedSequences`.

---

## Scan Output Format

`screenshots/<name>/scan.json`:
  ...
  "diverseProgressions": [
    {
      "steps": [
        { "fold": 0,  "pov": "iso" },
        { "fold": 25, "pov": [0.82, 0.54, 0.2] },
        { "fold": 50, "pov": [0.71, 0.56, -0.42] },
        { "fold": 75, "pov": [0.63, 0.58, -0.51] },
        { "fold": 90, "pov": [0.58, 0.59, -0.56] }
      ],
      "score": 0.78,
      "metrics": { "endAngle": 1.05, "totalAngle": 2.1 }
    }
  ],
  "progressionStats": {
    "trackingEvalMode": "strictAllSteps",
    "rejectedByQuality": 12,
    "rejectedByTrackedPoints": 7,
    "validBeforeDiversity": 18,
    "selectedAfterDiversity": 7
  },
  "povAnalysis": {
    "iso": {
      "alwaysVisibleFaceIds": [9, 10],
...
5. Greedy algorithm: build multi-POV sequences maximising min-quality per step
        ↓
6. Read `suggestedSequences` (and optionally `diverseProgressions` / `progressionStats`) in `screenshots/<name>/scan.json`
        ↓
...
| `screenshots/<name>/scan.json` | Scan output — POV analysis, suggested sequences, and optional diverse progressions |
